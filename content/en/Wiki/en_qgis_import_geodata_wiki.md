...
3. Click on the `?` in the Layers window. The window `Coordinate Reference System Selector` will open → Select the correct reference system → Click `OK`.
...
3. In the window "Data Source Manager | Delimited Text" in QGIS you can find multiple drop-down menus
    ...
    * `Geometry definition`: In this section, you specify which columns of the file contain the spatial information to geo-reference the data on the map. If the file has a column containing __latitude__ and another with __longitude__ data, you can use them to georeference the data. Check `Point Coordinates`. Select for `X field` “LONGITUDE” and for `Y field` “LATITUDE”.
...
4. Select your table as `Input Layer`.
