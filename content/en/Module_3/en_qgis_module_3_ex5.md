...
2. Once the project is created [save the project](../Wiki/en_qgis_projects_folder_structure_wiki.md#save-project) in the folder of the exercise "Module_3_Exercise_2_Flood_Larkana". To do that click on `Project` → `Save as` and navigate to the folder. Name the project "PAK_Larkana_flood_2024".
...
4. Next, load the GeoPackage __"PAK_Sindh_adm2.gpkg"__ in your project by drag and drop ([Wiki Video](../Wiki/en_qgis_import_geodata_wiki.md#open-raster-data-via-drag-and-drop)). Or click on `Layer` → `Add Layer` → `Add Vector Layer`. Click on the three points ![](../../../fig/Three_points.png) and navigate to __"PAK_Sindh_adm2.gpkg"__. Select the file and click `Open`. Back in QGIS click `Add` ([Wiki Video](../Wiki/en_qgis_import_geodata_wiki.md#open-vector-data-via-layer-tab)).
...
5. To get an overview of the situation, we want to export the administrative boundaries for our Area of Interest (AOI). To do this, we want to export the district __Larkana__, as well as the neighbouring districts from the `PAK_adm2_Sindh`-layer. 
    - <kbd>Right-Click</kbd> on the `PAK_adm2_Sindh`-layer and select [Open Attribute table](../Wiki/en_qgis_attribute_table_wiki.md).
...
1. First, we need to find out where the health facilities are located in the area. We can find datasets by doing a quick search on the [humanitarian data exchange (HDX)](https://data.humdata.org). Here you can find the dataset "Pakistan Health Facilities (OpenStreetMap Export)". We will be using this dataset. The dataset is already available in the download folder for this exercise. 
    ...
    - In the __Processing Toolbox__ ([opening the toolbox](../Wiki/en_qgis_interface_wiki.md#open-toolbox)), search for the tool "Extract by Location". <kbd>Double-Click</kbd> on it. A new window will open.
...
4. This will be a problem as soon as we do something different then just displaying the layers. Since we want to manipulate the layers in the next step, we need to reproject them first ([Wiki Video](../Wiki/en_qgis_projections_wiki.md#changing-the-projection-of-a-vector-layer)). 
...
5. Open the `Processing Toolbox` ([Wiki video](../Wiki/en_qgis_interface_wiki.md#open-toolbox)) and search for the tool __"Select by Location"__.
...
* To do so open the [`Processing Toolbox`](../Wiki/en_qgis_interface_wiki.md#open-toolbox) and search for the tool __"Fix Geometries"__.
...
6.  We can edit the attribute table for the features that we have selected:
    - Open the attribute table of __"Health_Facilities_Flood_2024_AOI"__ by right clicking on the layer → `Open Attribute Table` ([Wiki Video](../Wiki/en_qgis_attribute_table_wiki.md)).
    - Activate the editing mode by clicking on ![](../../../fig/mActionToggleEditing.png) ([Wiki Video](../Wiki/en_qgis_attribute_table_wiki.md#change-data-in-the-attribute-table)). Now you are able to edit the data directly in the table.
    ...
    - In the `Add field`-window, add the name of the column and specify the `Type` to `Text (string)` ([Wiki Video](../Wiki/en_qgis_attribute_table_wiki.md#add-new-column)). 
    - Click `OK`. 
...
7. Next, we want to edit the rows in the attribute table for the features we have selected. 
    ...
    - Select the option `Show selected features` ([Wiki Video](../Wiki/en_qgis_attribute_table_wiki.md#manually-select-features-in-the-attribute-table)). This will filter the table to display only the rows that represent the health facilities directly impacted by the flood.
...
::::{margin}
:::{tip}
There are methods to automate the digitisation process which will be covered in [module 5: Intermediate GIS Operations](../Module_5/en_module_5_overview.md).
:::
::::
...
4. Now you can create a point for each place where the flood layer covers the main roads leading out of Larkana ([Wiki](../Wiki/en_qgis_digitisation_wiki.md#creation-of-point-data)). 
...
5. Now, we have mapped all blocked main access roads into Larkana. We can use icons instead of just points to display the layer __“PAK_flood_2024_blocked_road”__ to visualise this fact better ([Wiki](../Wiki/en_qgis_single_symbol_wiki.md)).
...
Click [here](../Module_4/en_qgis_map_design_I_ex2.md) if you want to continue to the next exercise of this exercise track.
