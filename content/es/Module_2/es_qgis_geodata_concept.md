...
- Las entidades geográficas se exponen en los mapas con una representación geométrica, pero se componen de información organizada en tablas (véase {numref}`es_Geodata_attribute_table_example`).
...
En {numref}`es_Vector` y {numref}`es_Raster` puede ver la misma ubicación, a la izquierda como datos vectoriales, con la visualización de las calles y de la zona urbana y a la derecha, como datos ráster (imagen satelital), que muestra la cubierta terrestre.
...
- En la barra superior, utilice el menú `Capa` → `Añadir capa vectorial`.
