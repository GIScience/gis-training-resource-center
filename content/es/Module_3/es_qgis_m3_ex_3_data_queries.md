...
:::{grid-item-card}
:link: es_qgis_module_3_exercises.md
...
* [Tipos de datos geográficos](../Wiki/es_qgis_geodata_types_wiki.md)
* [Importación de datos geográficos en QGIS](../Wiki/es_qgis_import_geodata_wiki.md)
* [Concepto de capa](../Wiki/es_qgis_layer_concept_wiki.md)
* [Consultas no espaciales](../Wiki/es_qgis_non_spatial_queries_wiki.md)
...
* [Geoprocesamiento: recorte](../Wiki/es_qgis_geoprocessing_wiki.md)
...
La carpeta se llama **Module_3_Exercise_3_Data_Queries** y contiene toda la [estructura de carpetas estándar](../Wiki/es_qgis_projects_folder_structure_wiki.md) con todos los datos de la carpeta de entrada.
...
2. Una vez que se haya creado el proyecto, [guarde el proyecto](../Wiki/es_qgis_projects_folder_structure_wiki.md) en la **carpeta del proyecto** del ejercicio **Module_3_Exercise_1_Queries_Somalia**. Para hacer esto, haga clic en `Proyecto` → `Guardar como` e ir hasta la carpeta. Nombre el proyecto de esta manera: **Somalia_flood_affected_Beledweyne_2023**.

3. Para cargar los siguientes archivos en su proyecto, arrastre y suelte ([video en Wiki](../Wiki/es_qgis_import_geodata_wiki.md)). O haga clic en `Capa` → `Añadir capa` → `Añadir capa vectorial`. Haga clic en los tres puntos ![](../../../fig/Three_points.png) y navegue hasta el archivo. Seleccione el archivo y hacer clic en `Abrir`. De vuelta en QGIS, haga clic en `Añadir` ([Wiki Video](../Wiki/es_qgis_import_geodata_wiki.md)).
...
4. En primer lugar, queremos exportar el distrito __Beledweyne__ de la región de Hiraan desde `som_admbnda_adm2_ocha_20230308.shp` para tenerlo como capa vectorial independiente. Para hacer esto:
    1. Abra la tabla de atributos de `som_admbnda_adm2_ocha_20230308.shp` al hacer clic con el botón derecho en la capa --> `Abrir tabla de atributos`([video en Wiki](../Wiki/es_qgis_attribute_table_wiki.md)).
    2. Busque la fila de `Belet Weyne` y márquela al hacer clic en el número que está en el extremo izquierdo de la tabla de atributos. La fila se resaltará en azul y el distrito se volverá amarillo en el lienzo del mapa. Puede hacer clic con el botón derecho en la fila y hacer clic en `Acercar mapa el mapa a las filas seleccionadas`([Wiki Video](../Wiki/es_qgis_attribute_table_wiki.md)).
    3. Ahora haga clic derecho en la capa en el Panel de Capas y luego haga clic en `Exportar` → `Guardar objetos seleccionados como`. Queremos guardar Beledweyne como GeoPackage, así que ajuste `Formato` en consecuencia. Haga clic en los tres puntos y navegue a su **carpeta temporal**. Aquí puede asignar a la capa el nombre **AOI_Beledweyne** y hacer clic en `Guardar`. Ahora haga clic en `Aceptar`([Wiki Video](../Wiki/es_qgis_non_spatial_queries_wiki.md)). En este ejercicio, no reproyectaremos las capas y trabajaremos con los datos en `ESPG:4326 - WGS84`.
...
5. En los pasos siguientes, queremos identificar todos los edificios que puedan verse afectados por las recientes inundaciones. Para hacer esto utilizaremos la herramienta `Extraer por ubicación`.
    ...
    :::{Attention}
    La herramienta [`Seleccionar por ubicación`](../Wiki/es_qgis_spatial_queries_wiki.md) es muy similar. Esta herramienta funciona de la misma manera, pero en lugar de extraer directamente las entidades, las selecciona.
    :::
...
Para extraer "hospitales", "escuelas" y "mezquitas", podemos utilizar la herramienta `Extraer por expresión`.
    1.  Busque la herramienta `Extraer por expresión` en la __[caja de herramientas de procesos](../Wiki/es_qgis_interface_wiki.md)__.
