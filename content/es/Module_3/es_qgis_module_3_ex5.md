...
Este es el tercer ejercicio en el [Programa de ejercicios de respuesta a las inundaciones en Larkana:](../Exercise_tracks/es_larkana_flood_response.md).

- [Ejercicio anterior](../Module_2/es_qgis_data_sources_ex4.md)
- [Siguiente ejercicio](../Module_4/es_qgis_map_design_I_ex2.md)
:::
...
:::
::::
1. Abra QGIS y cree un [nuevo proyecto](../Wiki/es_qgis_projects_folder_structure_wiki.md) haciendo clic en `Proyecto` → `Nuevo proyecto`.
2. Una vez creado el proyecto [guarde el proyecto](../Wiki/es_qgis_projects_folder_structure_wiki.md) en la carpeta del ejercicio “Modul3_Exercise_2_Flood_Larkana”. Para hacer esto, haga clic en `Proyecto` → `Guardar como` y navegue hasta la carpeta. Nombre el proyecto “PAK_Larkana_flood_2024”.
...
4. A continuación, cargue el GeoPackage __“PAK_Sindh_adm2.gpkg”__ en su proyecto. Para hacerlo, debe arrastrar y soltar ([video en Wiki](../Wiki/es_qgis_import_geodata_wiki.md)). O haga clic en `Capa` → `Añadir capa` → `Añadir capa vectorial`. Haga clic en los tres puntos ![](../../../fig/Three_points.png) de tres puntos y navegue hasta __"PAK_Sindh_adm2.gpkg"__. Seleccione el archivo y hacer clic en `Abrir`. De vuelta en QGIS, haga clic en `Añadir` ([Wiki Video](../Wiki/es_qgis_import_geodata_wiki.md)).
...
5. Para obtener una visión general de la situación, debemos exportar los límites administrativos para nuestra Área de Interés (AOI). Para hacerlo, debemos exportar el distrito de __Larkana__, así como los distritos vecinos, desde la capa `PAK_adm2_Sindh`.
    - Haga <kbd>clic derecho</kbd> en la capa `PAK_adm2_Sindh` y seleccionar [Abrir tabla de atributos](../Wiki/es_qgis_attribute_table_wiki.md).
    ...
    ::::
    - En la barra de herramientas en la parte superior de la ventana de QGIS, seleccione la herramienta `Seleccionar objeto(s) espacial(es)` ![](../../../fig/selection_toolbar_feature_selection.png). Mantenga presionado <kbd>Ctrl</kbd> y <kbd>haga clic</kbd> en los distritos que rodean el distrito de Larkana ([video en Wiki](../../en/Wiki/en_qgis_spatial_queries_wiki.md#manual-selection)). Los seis distritos seleccionados se verán de color amarillo en el lienzo del mapa.
...
1. En primer lugar, debemos averiguar dónde se encuentran los centros de salud en la zona. Podemos encontrar conjuntos de datos mediante una búsqueda rápida en el [Intercambio de Datos Humanitarios (HDX)](https://data.humdata.org). Allí puede encontrarse el conjunto de datos denominado “Centros de salud de Pakistán (exportado de OpenStreetMap)”. Usaremos este conjunto de datos. El conjunto de datos ya está disponible en la carpeta de descargas para este ejercicio.
    - Importe el GeoPackage `PAK_Health_Facilities_complete.gpgk` en su proyecto. Puede arrastrarlo al lienzo del mapa o abrir la ventana de importación haciendo clic en `Capa` → `Añadir capa` → `Añadir capa vectorial` en la barra superior de QGIS ([consulte la página](../Wiki/es_qgis_import_geodata_wiki.md) Wiki). Aparecerá una nueva capa con datos de punto en el lienzo del mapa.
    ...
    - En la __caja de herramientas de Procesos__ ([abrir la caja](../Wiki/es_qgis_interface_wiki.md)), busque la herramienta “Extraer por ubicación”. Haga <kbd>doble clic</kbd> en ella. Se abrirá una nueva ventana.
...
4. Esto se convertirá en un problema tan pronto como hagamos algo diferente de solo mostrar las capas. Ya que queremos manipular las capas en el siguiente paso, primero necesitamos reproyectarlas ([video en Wiki](../Wiki/es_qgis_projections_wiki.md)).
...
5. Abra el caja de herramientas de procesos ([video en Wiki](../Wiki/es_qgis_interface_wiki.md)) y busque la herramienta __“Seleccionar por ubicación”__.
...
* Para ello, abra el [`Processing Toolbox`](../Wiki/es_qgis_interface_wiki.md) y busque la herramienta __“Arreglar geometrías”__.
...
6. Podemos editar la tabla de atributos para las entidades que hemos seleccionado:
    - Abra la tabla de atributos de __“Health_Facilities_Flood_2024_AOI”__ haciendo clic derecho sobre la capa -> `Abrir tabla de atributos` ([video en Wiki](../Wiki/es_qgis_attribute_table_wiki.md))
    - Active el modo de edición haciendo clic en ![](../../../fig/mActionToggleEditing.png) ([video en Wiki](../Wiki/es_qgis_attribute_table_wiki.md)). Ahora puede editar los datos directamente en la tabla.
    ...
    - En la ventana de `Añadir campo`, agregue el nombre de la columna y especifique el `Tipo` a `Texto (cadena)` ([video en Wiki](../Wiki/es_qgis_attribute_table_wiki.md)).
...
7. A continuación, queremos editar las filas de la tabla de atributos para las entidades que hemos seleccionado.
    ...
    - Seleccione la opción `Mostrar objetos espaciales seleccionados` ([video en Wiki](../../en/Wiki/en_qgis_attribute_table_wiki.md#manually-select-features-in-the-attribute-table)). Esto filtrará la tabla para mostrar solo las filas que representan los centros de salud directamente afectados por la inundación.
...
2. Configuremos la clasificación por categorías. En los datos de OpenStreetMap se distingue entre diferentes tipos de carreteras utilizando la columna “autopista”.
    ...
    - Vaya a la pestaña `Simbología`.
    - En la parte superior, seleccione `Categorizado` ([video en Wiki](../Wiki/es_qgis_categorised_wiki.md)).
...
::::{margin}
:::{tip}
Existen métodos para automatizar el proceso de digitalización que se tratarán en el [módulo 5: Operaciones intermedias del SIG](../Module_5/es_module_5_overview.md).
:::
::::
...
::::{margin}
:::{tip}
Si no puede ver la barra de herramientas, haga clic en la pestaña `Ver` → `Barras de herramientas` y marque `Digitalización` ([video en Wiki](../Wiki/es_qgis_digitisation_wiki.md)).
:::
::::

4. Ahora puede crear un punto para cada lugar donde la capa de inundación cubre las carreteras principales que conducen a Larkana ([Wiki](../Wiki/es_qgis_digitisation_wiki.md)).
...
5. Ahora, ya hemos mapeado todas las carreteras principales bloqueadas de acceso a Larkana. Podemos usar iconos en lugar de solo puntos para mostrar la capa __“PAK_flood_2024_blocked_road”__ y visualizar mejor este hecho ([Wiki](../Wiki/es_qgis_single_symbol_wiki.md)).
...
Haga clic [aquí](../Module_4/es_qgis_map_design_I_ex2.md) si desea continuar con el siguiente ejercicio de este programa.
