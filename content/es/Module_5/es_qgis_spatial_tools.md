...
:::{tip}
Puede encontrar y activar el panel de la caja de herramientas de Procesos navegando a `Ver` → `Paneles` → `Caja de herramientas de Procesos`. Debería aparecer en la parte derecha de la pantalla.
:::

::::

::::{dropdown} __Herramienta:__ Unir atributos por localización
...
- La primera capa de entrada (véase "Unir a entidades en" en {numref}`es_spatial_join_1`) dicta qué entidades geométricas se copiarán en la nueva capa.
...
5. Utilice `están dentro` como predicado geométrico.
6. Seleccione los campos que desea añadir: `ADM2_EN`, `ADM2_PCODE`.
7. Seleccione `Descartar registros que no se pudieron unir`.
8. Haga clic en `Ejecutar` para continuar; el registro debería confirmar que se ha realizado correctamente.
9. Aparecerá una nueva capa (temporal) denominada "capa unida" en el panel de capas.
...
- La primera, y la más común, es la herramienta __"Unir atributos por localización"__ que hemos utilizado en el ejercicio anterior.
- También existen las herramientas __"Unir atributos por localización (resumen)"__ y __"Unir atributos por proximidad"__.

:::::{tab-set}

::::{tab-item} Unir atributos por localización (resumen)
...
::::{tab-item} Unir atributos por proximidad
...
2. Localice la herramienta denominada __Unir atributos por localización (resumen)__.
    - Elija __límites administrativas__ como capa de destino para unir entidades.
    - Establezca __intersecan__ como relación espacial.
    ...
    - Seleccione __suma__ como tipo de resúmenes a calcular.
    - Haga clic en `Ejecutar` para iniciar el proceso.
...
- __Recorte:__ Utiliza una capa (la capa de "recortado") para recortar o cortar otra capa de modo que solo quede la parte de la capa de entrada que cae dentro (o quizás fuera) de la capa de recorte. Extrae eficazmente un subconjunto de geometría.
