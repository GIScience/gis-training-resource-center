...
En la ventana del Modelador gráfico se pueden ver varios iconos y menús. En primer lugar, nos centraremos en la ventana izquierda de la sección `Entradas` y `Algoritmos`. Las entradas son todas las variables o capas de entrada de un modelo, como una capa vectorial, una capa rasterizada, una cadena, una booleana, una expresión y muchas otras. Los algoritmos son todas las herramientas que se utilizan para procesar las variables de entrada. En la cadena de procesamiento, un algoritmo/herramienta devolverá una salida que será utilizada por otra/herramienta siguiente hasta crear la salida final.
...
1. Seleccione la `Entrada` pestaña en la ventana de la izquierda.
2. A continuación, seleccione la página `Capa vectorial` haciendo doble clic o arrastrándola y soltándola en el lienzo del modelo ({numref}`es_vector_layer_modeler`). Se abrirá la ventana de definición de parámetros de entrada ({numref}`es_input_vector_modeler`).
3. En esta ventana podemos personalizar algunos parámetros de entrada vectorial como `Descripción`, `Tipo de geometría` (Punto, Línea, Polígono) y también podemos definir nuestra entrada como obligatoria para su modelo marcando la casilla `Mandatory`.
...
1. Seleccione la `Algoritmos` pestaña de la izquierda.
...
- En la barra superior, vaya a `Editar` -> `Añadir capa de grupo`. Aparecerá un cuadro gris en el fondo del lienzo del modelo
